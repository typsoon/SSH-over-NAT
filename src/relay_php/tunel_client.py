...
def local_to_http(sock, my_conn_id):
...
def http_to_local(sock, my_conn_id):
    global current_conn_id
...
def start_client():
    ...
    while True:
        ...
        t1 = threading.Thread(target=local_to_http, args=(sock, my_conn_id), daemon=True)
        t2 = threading.Thread(target=http_to_local, args=(sock, my_conn_id), daemon=True)
        t1.start()
        t2.start()
